...
public class VideoOutput: NSObject {
  ...
  private let sharedPiPBridge = MoonfinPiPSharedBridge.shared
  ...
  private func _updateCallback() {
    ...
    if sharedPiPBridge.shouldEmitFrame(textureId: textureId) {
      sharedPiPBridge.emitFrame(texture: texture, textureId: textureId)
    }
